/*
Graphiant APIs

Graphiant API documentation.

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package graphiant_sdk

import (
	"encoding/json"
)

// checks if the AlertserviceTimeWindow type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AlertserviceTimeWindow{}

// AlertserviceTimeWindow struct for AlertserviceTimeWindow
type AlertserviceTimeWindow struct {
	BucketSizeSec *int32 `json:"bucketSizeSec,omitempty"`
	OldTs *GoogleProtobufTimestamp `json:"oldTs,omitempty"`
	RecentTs *GoogleProtobufTimestamp `json:"recentTs,omitempty"`
}

// NewAlertserviceTimeWindow instantiates a new AlertserviceTimeWindow object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAlertserviceTimeWindow() *AlertserviceTimeWindow {
	this := AlertserviceTimeWindow{}
	return &this
}

// NewAlertserviceTimeWindowWithDefaults instantiates a new AlertserviceTimeWindow object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAlertserviceTimeWindowWithDefaults() *AlertserviceTimeWindow {
	this := AlertserviceTimeWindow{}
	return &this
}

// GetBucketSizeSec returns the BucketSizeSec field value if set, zero value otherwise.
func (o *AlertserviceTimeWindow) GetBucketSizeSec() int32 {
	if o == nil || IsNil(o.BucketSizeSec) {
		var ret int32
		return ret
	}
	return *o.BucketSizeSec
}

// GetBucketSizeSecOk returns a tuple with the BucketSizeSec field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AlertserviceTimeWindow) GetBucketSizeSecOk() (*int32, bool) {
	if o == nil || IsNil(o.BucketSizeSec) {
		return nil, false
	}
	return o.BucketSizeSec, true
}

// HasBucketSizeSec returns a boolean if a field has been set.
func (o *AlertserviceTimeWindow) HasBucketSizeSec() bool {
	if o != nil && !IsNil(o.BucketSizeSec) {
		return true
	}

	return false
}

// SetBucketSizeSec gets a reference to the given int32 and assigns it to the BucketSizeSec field.
func (o *AlertserviceTimeWindow) SetBucketSizeSec(v int32) {
	o.BucketSizeSec = &v
}

// GetOldTs returns the OldTs field value if set, zero value otherwise.
func (o *AlertserviceTimeWindow) GetOldTs() GoogleProtobufTimestamp {
	if o == nil || IsNil(o.OldTs) {
		var ret GoogleProtobufTimestamp
		return ret
	}
	return *o.OldTs
}

// GetOldTsOk returns a tuple with the OldTs field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AlertserviceTimeWindow) GetOldTsOk() (*GoogleProtobufTimestamp, bool) {
	if o == nil || IsNil(o.OldTs) {
		return nil, false
	}
	return o.OldTs, true
}

// HasOldTs returns a boolean if a field has been set.
func (o *AlertserviceTimeWindow) HasOldTs() bool {
	if o != nil && !IsNil(o.OldTs) {
		return true
	}

	return false
}

// SetOldTs gets a reference to the given GoogleProtobufTimestamp and assigns it to the OldTs field.
func (o *AlertserviceTimeWindow) SetOldTs(v GoogleProtobufTimestamp) {
	o.OldTs = &v
}

// GetRecentTs returns the RecentTs field value if set, zero value otherwise.
func (o *AlertserviceTimeWindow) GetRecentTs() GoogleProtobufTimestamp {
	if o == nil || IsNil(o.RecentTs) {
		var ret GoogleProtobufTimestamp
		return ret
	}
	return *o.RecentTs
}

// GetRecentTsOk returns a tuple with the RecentTs field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AlertserviceTimeWindow) GetRecentTsOk() (*GoogleProtobufTimestamp, bool) {
	if o == nil || IsNil(o.RecentTs) {
		return nil, false
	}
	return o.RecentTs, true
}

// HasRecentTs returns a boolean if a field has been set.
func (o *AlertserviceTimeWindow) HasRecentTs() bool {
	if o != nil && !IsNil(o.RecentTs) {
		return true
	}

	return false
}

// SetRecentTs gets a reference to the given GoogleProtobufTimestamp and assigns it to the RecentTs field.
func (o *AlertserviceTimeWindow) SetRecentTs(v GoogleProtobufTimestamp) {
	o.RecentTs = &v
}

func (o AlertserviceTimeWindow) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AlertserviceTimeWindow) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.BucketSizeSec) {
		toSerialize["bucketSizeSec"] = o.BucketSizeSec
	}
	if !IsNil(o.OldTs) {
		toSerialize["oldTs"] = o.OldTs
	}
	if !IsNil(o.RecentTs) {
		toSerialize["recentTs"] = o.RecentTs
	}
	return toSerialize, nil
}

type NullableAlertserviceTimeWindow struct {
	value *AlertserviceTimeWindow
	isSet bool
}

func (v NullableAlertserviceTimeWindow) Get() *AlertserviceTimeWindow {
	return v.value
}

func (v *NullableAlertserviceTimeWindow) Set(val *AlertserviceTimeWindow) {
	v.value = val
	v.isSet = true
}

func (v NullableAlertserviceTimeWindow) IsSet() bool {
	return v.isSet
}

func (v *NullableAlertserviceTimeWindow) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAlertserviceTimeWindow(val *AlertserviceTimeWindow) *NullableAlertserviceTimeWindow {
	return &NullableAlertserviceTimeWindow{value: val, isSet: true}
}

func (v NullableAlertserviceTimeWindow) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAlertserviceTimeWindow) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


