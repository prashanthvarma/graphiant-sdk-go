/*
Graphiant APIs

Graphiant API documentation.

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package graphiant_sdk

import (
	"encoding/json"
)

// checks if the V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest{}

// V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest struct for V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest
type V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest struct {
	Id *int64 `json:"id,omitempty"`
	IsProvider *bool `json:"isProvider,omitempty"`
	TimeWindow *V2NotificationlistPostRequestTimeWindow `json:"timeWindow,omitempty"`
}

// NewV1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest instantiates a new V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewV1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest() *V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest {
	this := V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest{}
	return &this
}

// NewV1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequestWithDefaults instantiates a new V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewV1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequestWithDefaults() *V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest {
	this := V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) GetId() int64 {
	if o == nil || IsNil(o.Id) {
		var ret int64
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) GetIdOk() (*int64, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given int64 and assigns it to the Id field.
func (o *V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) SetId(v int64) {
	o.Id = &v
}

// GetIsProvider returns the IsProvider field value if set, zero value otherwise.
func (o *V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) GetIsProvider() bool {
	if o == nil || IsNil(o.IsProvider) {
		var ret bool
		return ret
	}
	return *o.IsProvider
}

// GetIsProviderOk returns a tuple with the IsProvider field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) GetIsProviderOk() (*bool, bool) {
	if o == nil || IsNil(o.IsProvider) {
		return nil, false
	}
	return o.IsProvider, true
}

// HasIsProvider returns a boolean if a field has been set.
func (o *V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) HasIsProvider() bool {
	if o != nil && !IsNil(o.IsProvider) {
		return true
	}

	return false
}

// SetIsProvider gets a reference to the given bool and assigns it to the IsProvider field.
func (o *V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) SetIsProvider(v bool) {
	o.IsProvider = &v
}

// GetTimeWindow returns the TimeWindow field value if set, zero value otherwise.
func (o *V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) GetTimeWindow() V2NotificationlistPostRequestTimeWindow {
	if o == nil || IsNil(o.TimeWindow) {
		var ret V2NotificationlistPostRequestTimeWindow
		return ret
	}
	return *o.TimeWindow
}

// GetTimeWindowOk returns a tuple with the TimeWindow field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) GetTimeWindowOk() (*V2NotificationlistPostRequestTimeWindow, bool) {
	if o == nil || IsNil(o.TimeWindow) {
		return nil, false
	}
	return o.TimeWindow, true
}

// HasTimeWindow returns a boolean if a field has been set.
func (o *V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) HasTimeWindow() bool {
	if o != nil && !IsNil(o.TimeWindow) {
		return true
	}

	return false
}

// SetTimeWindow gets a reference to the given V2NotificationlistPostRequestTimeWindow and assigns it to the TimeWindow field.
func (o *V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) SetTimeWindow(v V2NotificationlistPostRequestTimeWindow) {
	o.TimeWindow = &v
}

func (o V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.IsProvider) {
		toSerialize["isProvider"] = o.IsProvider
	}
	if !IsNil(o.TimeWindow) {
		toSerialize["timeWindow"] = o.TimeWindow
	}
	return toSerialize, nil
}

type NullableV1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest struct {
	value *V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest
	isSet bool
}

func (v NullableV1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) Get() *V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest {
	return v.value
}

func (v *NullableV1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) Set(val *V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableV1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableV1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableV1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest(val *V1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) *NullableV1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest {
	return &NullableV1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest{value: val, isSet: true}
}

func (v NullableV1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableV1ExtranetB2bMonitoringPeeringServiceServiceCustomerListPostRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


