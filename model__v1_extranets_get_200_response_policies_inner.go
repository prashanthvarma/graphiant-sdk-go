/*
Graphiant APIs

Graphiant API documentation.

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package graphiant_sdk

import (
	"encoding/json"
)

// checks if the V1ExtranetsGet200ResponsePoliciesInner type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &V1ExtranetsGet200ResponsePoliciesInner{}

// V1ExtranetsGet200ResponsePoliciesInner struct for V1ExtranetsGet200ResponsePoliciesInner
type V1ExtranetsGet200ResponsePoliciesInner struct {
	Auto *V1ExtranetsGet200ResponsePoliciesInnerAuto `json:"auto,omitempty"`
	Branches *V1ExtranetsGet200ResponsePoliciesInnerBranches `json:"branches,omitempty"`
	CreatedAt *V1AlarmHistoryGet200ResponseHistoryInnerTime `json:"createdAt,omitempty"`
	CreatedBy *string `json:"createdBy,omitempty"`
	Description *string `json:"description,omitempty"`
	HostPrefixSet *V1ExtranetsGet200ResponsePoliciesInnerBranchesPrefixSet `json:"hostPrefixSet,omitempty"`
	Id *int64 `json:"id,omitempty"`
	Manual *V1ExtranetsGet200ResponsePoliciesInnerManual `json:"manual,omitempty"`
	Name *string `json:"name,omitempty"`
	SharedPrefixes []string `json:"sharedPrefixes,omitempty"`
	SharedSegment *V1ExtranetsGet200ResponsePoliciesInnerBranchesExcludedDevicesInnerSegmentsInner `json:"sharedSegment,omitempty"`
	Source *V1ExtranetsGet200ResponsePoliciesInnerBranches `json:"source,omitempty"`
	TargetSegments []V1ExtranetsGet200ResponsePoliciesInnerBranchesExcludedDevicesInnerSegmentsInner `json:"targetSegments,omitempty"`
	Type *string `json:"type,omitempty"`
	UpdatedAt *V1AlarmHistoryGet200ResponseHistoryInnerTime `json:"updatedAt,omitempty"`
}

// NewV1ExtranetsGet200ResponsePoliciesInner instantiates a new V1ExtranetsGet200ResponsePoliciesInner object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewV1ExtranetsGet200ResponsePoliciesInner() *V1ExtranetsGet200ResponsePoliciesInner {
	this := V1ExtranetsGet200ResponsePoliciesInner{}
	return &this
}

// NewV1ExtranetsGet200ResponsePoliciesInnerWithDefaults instantiates a new V1ExtranetsGet200ResponsePoliciesInner object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewV1ExtranetsGet200ResponsePoliciesInnerWithDefaults() *V1ExtranetsGet200ResponsePoliciesInner {
	this := V1ExtranetsGet200ResponsePoliciesInner{}
	return &this
}

// GetAuto returns the Auto field value if set, zero value otherwise.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetAuto() V1ExtranetsGet200ResponsePoliciesInnerAuto {
	if o == nil || IsNil(o.Auto) {
		var ret V1ExtranetsGet200ResponsePoliciesInnerAuto
		return ret
	}
	return *o.Auto
}

// GetAutoOk returns a tuple with the Auto field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetAutoOk() (*V1ExtranetsGet200ResponsePoliciesInnerAuto, bool) {
	if o == nil || IsNil(o.Auto) {
		return nil, false
	}
	return o.Auto, true
}

// HasAuto returns a boolean if a field has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) HasAuto() bool {
	if o != nil && !IsNil(o.Auto) {
		return true
	}

	return false
}

// SetAuto gets a reference to the given V1ExtranetsGet200ResponsePoliciesInnerAuto and assigns it to the Auto field.
func (o *V1ExtranetsGet200ResponsePoliciesInner) SetAuto(v V1ExtranetsGet200ResponsePoliciesInnerAuto) {
	o.Auto = &v
}

// GetBranches returns the Branches field value if set, zero value otherwise.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetBranches() V1ExtranetsGet200ResponsePoliciesInnerBranches {
	if o == nil || IsNil(o.Branches) {
		var ret V1ExtranetsGet200ResponsePoliciesInnerBranches
		return ret
	}
	return *o.Branches
}

// GetBranchesOk returns a tuple with the Branches field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetBranchesOk() (*V1ExtranetsGet200ResponsePoliciesInnerBranches, bool) {
	if o == nil || IsNil(o.Branches) {
		return nil, false
	}
	return o.Branches, true
}

// HasBranches returns a boolean if a field has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) HasBranches() bool {
	if o != nil && !IsNil(o.Branches) {
		return true
	}

	return false
}

// SetBranches gets a reference to the given V1ExtranetsGet200ResponsePoliciesInnerBranches and assigns it to the Branches field.
func (o *V1ExtranetsGet200ResponsePoliciesInner) SetBranches(v V1ExtranetsGet200ResponsePoliciesInnerBranches) {
	o.Branches = &v
}

// GetCreatedAt returns the CreatedAt field value if set, zero value otherwise.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetCreatedAt() V1AlarmHistoryGet200ResponseHistoryInnerTime {
	if o == nil || IsNil(o.CreatedAt) {
		var ret V1AlarmHistoryGet200ResponseHistoryInnerTime
		return ret
	}
	return *o.CreatedAt
}

// GetCreatedAtOk returns a tuple with the CreatedAt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetCreatedAtOk() (*V1AlarmHistoryGet200ResponseHistoryInnerTime, bool) {
	if o == nil || IsNil(o.CreatedAt) {
		return nil, false
	}
	return o.CreatedAt, true
}

// HasCreatedAt returns a boolean if a field has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) HasCreatedAt() bool {
	if o != nil && !IsNil(o.CreatedAt) {
		return true
	}

	return false
}

// SetCreatedAt gets a reference to the given V1AlarmHistoryGet200ResponseHistoryInnerTime and assigns it to the CreatedAt field.
func (o *V1ExtranetsGet200ResponsePoliciesInner) SetCreatedAt(v V1AlarmHistoryGet200ResponseHistoryInnerTime) {
	o.CreatedAt = &v
}

// GetCreatedBy returns the CreatedBy field value if set, zero value otherwise.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetCreatedBy() string {
	if o == nil || IsNil(o.CreatedBy) {
		var ret string
		return ret
	}
	return *o.CreatedBy
}

// GetCreatedByOk returns a tuple with the CreatedBy field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetCreatedByOk() (*string, bool) {
	if o == nil || IsNil(o.CreatedBy) {
		return nil, false
	}
	return o.CreatedBy, true
}

// HasCreatedBy returns a boolean if a field has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) HasCreatedBy() bool {
	if o != nil && !IsNil(o.CreatedBy) {
		return true
	}

	return false
}

// SetCreatedBy gets a reference to the given string and assigns it to the CreatedBy field.
func (o *V1ExtranetsGet200ResponsePoliciesInner) SetCreatedBy(v string) {
	o.CreatedBy = &v
}

// GetDescription returns the Description field value if set, zero value otherwise.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetDescription() string {
	if o == nil || IsNil(o.Description) {
		var ret string
		return ret
	}
	return *o.Description
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetDescriptionOk() (*string, bool) {
	if o == nil || IsNil(o.Description) {
		return nil, false
	}
	return o.Description, true
}

// HasDescription returns a boolean if a field has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) HasDescription() bool {
	if o != nil && !IsNil(o.Description) {
		return true
	}

	return false
}

// SetDescription gets a reference to the given string and assigns it to the Description field.
func (o *V1ExtranetsGet200ResponsePoliciesInner) SetDescription(v string) {
	o.Description = &v
}

// GetHostPrefixSet returns the HostPrefixSet field value if set, zero value otherwise.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetHostPrefixSet() V1ExtranetsGet200ResponsePoliciesInnerBranchesPrefixSet {
	if o == nil || IsNil(o.HostPrefixSet) {
		var ret V1ExtranetsGet200ResponsePoliciesInnerBranchesPrefixSet
		return ret
	}
	return *o.HostPrefixSet
}

// GetHostPrefixSetOk returns a tuple with the HostPrefixSet field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetHostPrefixSetOk() (*V1ExtranetsGet200ResponsePoliciesInnerBranchesPrefixSet, bool) {
	if o == nil || IsNil(o.HostPrefixSet) {
		return nil, false
	}
	return o.HostPrefixSet, true
}

// HasHostPrefixSet returns a boolean if a field has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) HasHostPrefixSet() bool {
	if o != nil && !IsNil(o.HostPrefixSet) {
		return true
	}

	return false
}

// SetHostPrefixSet gets a reference to the given V1ExtranetsGet200ResponsePoliciesInnerBranchesPrefixSet and assigns it to the HostPrefixSet field.
func (o *V1ExtranetsGet200ResponsePoliciesInner) SetHostPrefixSet(v V1ExtranetsGet200ResponsePoliciesInnerBranchesPrefixSet) {
	o.HostPrefixSet = &v
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetId() int64 {
	if o == nil || IsNil(o.Id) {
		var ret int64
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetIdOk() (*int64, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given int64 and assigns it to the Id field.
func (o *V1ExtranetsGet200ResponsePoliciesInner) SetId(v int64) {
	o.Id = &v
}

// GetManual returns the Manual field value if set, zero value otherwise.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetManual() V1ExtranetsGet200ResponsePoliciesInnerManual {
	if o == nil || IsNil(o.Manual) {
		var ret V1ExtranetsGet200ResponsePoliciesInnerManual
		return ret
	}
	return *o.Manual
}

// GetManualOk returns a tuple with the Manual field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetManualOk() (*V1ExtranetsGet200ResponsePoliciesInnerManual, bool) {
	if o == nil || IsNil(o.Manual) {
		return nil, false
	}
	return o.Manual, true
}

// HasManual returns a boolean if a field has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) HasManual() bool {
	if o != nil && !IsNil(o.Manual) {
		return true
	}

	return false
}

// SetManual gets a reference to the given V1ExtranetsGet200ResponsePoliciesInnerManual and assigns it to the Manual field.
func (o *V1ExtranetsGet200ResponsePoliciesInner) SetManual(v V1ExtranetsGet200ResponsePoliciesInnerManual) {
	o.Manual = &v
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *V1ExtranetsGet200ResponsePoliciesInner) SetName(v string) {
	o.Name = &v
}

// GetSharedPrefixes returns the SharedPrefixes field value if set, zero value otherwise.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetSharedPrefixes() []string {
	if o == nil || IsNil(o.SharedPrefixes) {
		var ret []string
		return ret
	}
	return o.SharedPrefixes
}

// GetSharedPrefixesOk returns a tuple with the SharedPrefixes field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetSharedPrefixesOk() ([]string, bool) {
	if o == nil || IsNil(o.SharedPrefixes) {
		return nil, false
	}
	return o.SharedPrefixes, true
}

// HasSharedPrefixes returns a boolean if a field has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) HasSharedPrefixes() bool {
	if o != nil && !IsNil(o.SharedPrefixes) {
		return true
	}

	return false
}

// SetSharedPrefixes gets a reference to the given []string and assigns it to the SharedPrefixes field.
func (o *V1ExtranetsGet200ResponsePoliciesInner) SetSharedPrefixes(v []string) {
	o.SharedPrefixes = v
}

// GetSharedSegment returns the SharedSegment field value if set, zero value otherwise.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetSharedSegment() V1ExtranetsGet200ResponsePoliciesInnerBranchesExcludedDevicesInnerSegmentsInner {
	if o == nil || IsNil(o.SharedSegment) {
		var ret V1ExtranetsGet200ResponsePoliciesInnerBranchesExcludedDevicesInnerSegmentsInner
		return ret
	}
	return *o.SharedSegment
}

// GetSharedSegmentOk returns a tuple with the SharedSegment field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetSharedSegmentOk() (*V1ExtranetsGet200ResponsePoliciesInnerBranchesExcludedDevicesInnerSegmentsInner, bool) {
	if o == nil || IsNil(o.SharedSegment) {
		return nil, false
	}
	return o.SharedSegment, true
}

// HasSharedSegment returns a boolean if a field has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) HasSharedSegment() bool {
	if o != nil && !IsNil(o.SharedSegment) {
		return true
	}

	return false
}

// SetSharedSegment gets a reference to the given V1ExtranetsGet200ResponsePoliciesInnerBranchesExcludedDevicesInnerSegmentsInner and assigns it to the SharedSegment field.
func (o *V1ExtranetsGet200ResponsePoliciesInner) SetSharedSegment(v V1ExtranetsGet200ResponsePoliciesInnerBranchesExcludedDevicesInnerSegmentsInner) {
	o.SharedSegment = &v
}

// GetSource returns the Source field value if set, zero value otherwise.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetSource() V1ExtranetsGet200ResponsePoliciesInnerBranches {
	if o == nil || IsNil(o.Source) {
		var ret V1ExtranetsGet200ResponsePoliciesInnerBranches
		return ret
	}
	return *o.Source
}

// GetSourceOk returns a tuple with the Source field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetSourceOk() (*V1ExtranetsGet200ResponsePoliciesInnerBranches, bool) {
	if o == nil || IsNil(o.Source) {
		return nil, false
	}
	return o.Source, true
}

// HasSource returns a boolean if a field has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) HasSource() bool {
	if o != nil && !IsNil(o.Source) {
		return true
	}

	return false
}

// SetSource gets a reference to the given V1ExtranetsGet200ResponsePoliciesInnerBranches and assigns it to the Source field.
func (o *V1ExtranetsGet200ResponsePoliciesInner) SetSource(v V1ExtranetsGet200ResponsePoliciesInnerBranches) {
	o.Source = &v
}

// GetTargetSegments returns the TargetSegments field value if set, zero value otherwise.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetTargetSegments() []V1ExtranetsGet200ResponsePoliciesInnerBranchesExcludedDevicesInnerSegmentsInner {
	if o == nil || IsNil(o.TargetSegments) {
		var ret []V1ExtranetsGet200ResponsePoliciesInnerBranchesExcludedDevicesInnerSegmentsInner
		return ret
	}
	return o.TargetSegments
}

// GetTargetSegmentsOk returns a tuple with the TargetSegments field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetTargetSegmentsOk() ([]V1ExtranetsGet200ResponsePoliciesInnerBranchesExcludedDevicesInnerSegmentsInner, bool) {
	if o == nil || IsNil(o.TargetSegments) {
		return nil, false
	}
	return o.TargetSegments, true
}

// HasTargetSegments returns a boolean if a field has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) HasTargetSegments() bool {
	if o != nil && !IsNil(o.TargetSegments) {
		return true
	}

	return false
}

// SetTargetSegments gets a reference to the given []V1ExtranetsGet200ResponsePoliciesInnerBranchesExcludedDevicesInnerSegmentsInner and assigns it to the TargetSegments field.
func (o *V1ExtranetsGet200ResponsePoliciesInner) SetTargetSegments(v []V1ExtranetsGet200ResponsePoliciesInnerBranchesExcludedDevicesInnerSegmentsInner) {
	o.TargetSegments = v
}

// GetType returns the Type field value if set, zero value otherwise.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetType() string {
	if o == nil || IsNil(o.Type) {
		var ret string
		return ret
	}
	return *o.Type
}

// GetTypeOk returns a tuple with the Type field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetTypeOk() (*string, bool) {
	if o == nil || IsNil(o.Type) {
		return nil, false
	}
	return o.Type, true
}

// HasType returns a boolean if a field has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) HasType() bool {
	if o != nil && !IsNil(o.Type) {
		return true
	}

	return false
}

// SetType gets a reference to the given string and assigns it to the Type field.
func (o *V1ExtranetsGet200ResponsePoliciesInner) SetType(v string) {
	o.Type = &v
}

// GetUpdatedAt returns the UpdatedAt field value if set, zero value otherwise.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetUpdatedAt() V1AlarmHistoryGet200ResponseHistoryInnerTime {
	if o == nil || IsNil(o.UpdatedAt) {
		var ret V1AlarmHistoryGet200ResponseHistoryInnerTime
		return ret
	}
	return *o.UpdatedAt
}

// GetUpdatedAtOk returns a tuple with the UpdatedAt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) GetUpdatedAtOk() (*V1AlarmHistoryGet200ResponseHistoryInnerTime, bool) {
	if o == nil || IsNil(o.UpdatedAt) {
		return nil, false
	}
	return o.UpdatedAt, true
}

// HasUpdatedAt returns a boolean if a field has been set.
func (o *V1ExtranetsGet200ResponsePoliciesInner) HasUpdatedAt() bool {
	if o != nil && !IsNil(o.UpdatedAt) {
		return true
	}

	return false
}

// SetUpdatedAt gets a reference to the given V1AlarmHistoryGet200ResponseHistoryInnerTime and assigns it to the UpdatedAt field.
func (o *V1ExtranetsGet200ResponsePoliciesInner) SetUpdatedAt(v V1AlarmHistoryGet200ResponseHistoryInnerTime) {
	o.UpdatedAt = &v
}

func (o V1ExtranetsGet200ResponsePoliciesInner) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o V1ExtranetsGet200ResponsePoliciesInner) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Auto) {
		toSerialize["auto"] = o.Auto
	}
	if !IsNil(o.Branches) {
		toSerialize["branches"] = o.Branches
	}
	if !IsNil(o.CreatedAt) {
		toSerialize["createdAt"] = o.CreatedAt
	}
	if !IsNil(o.CreatedBy) {
		toSerialize["createdBy"] = o.CreatedBy
	}
	if !IsNil(o.Description) {
		toSerialize["description"] = o.Description
	}
	if !IsNil(o.HostPrefixSet) {
		toSerialize["hostPrefixSet"] = o.HostPrefixSet
	}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.Manual) {
		toSerialize["manual"] = o.Manual
	}
	if !IsNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !IsNil(o.SharedPrefixes) {
		toSerialize["sharedPrefixes"] = o.SharedPrefixes
	}
	if !IsNil(o.SharedSegment) {
		toSerialize["sharedSegment"] = o.SharedSegment
	}
	if !IsNil(o.Source) {
		toSerialize["source"] = o.Source
	}
	if !IsNil(o.TargetSegments) {
		toSerialize["targetSegments"] = o.TargetSegments
	}
	if !IsNil(o.Type) {
		toSerialize["type"] = o.Type
	}
	if !IsNil(o.UpdatedAt) {
		toSerialize["updatedAt"] = o.UpdatedAt
	}
	return toSerialize, nil
}

type NullableV1ExtranetsGet200ResponsePoliciesInner struct {
	value *V1ExtranetsGet200ResponsePoliciesInner
	isSet bool
}

func (v NullableV1ExtranetsGet200ResponsePoliciesInner) Get() *V1ExtranetsGet200ResponsePoliciesInner {
	return v.value
}

func (v *NullableV1ExtranetsGet200ResponsePoliciesInner) Set(val *V1ExtranetsGet200ResponsePoliciesInner) {
	v.value = val
	v.isSet = true
}

func (v NullableV1ExtranetsGet200ResponsePoliciesInner) IsSet() bool {
	return v.isSet
}

func (v *NullableV1ExtranetsGet200ResponsePoliciesInner) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableV1ExtranetsGet200ResponsePoliciesInner(val *V1ExtranetsGet200ResponsePoliciesInner) *NullableV1ExtranetsGet200ResponsePoliciesInner {
	return &NullableV1ExtranetsGet200ResponsePoliciesInner{value: val, isSet: true}
}

func (v NullableV1ExtranetsGet200ResponsePoliciesInner) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableV1ExtranetsGet200ResponsePoliciesInner) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


