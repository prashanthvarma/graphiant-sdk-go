/*
Graphiant APIs

Graphiant API documentation.

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package graphiant_sdk

import (
	"encoding/json"
)

// checks if the V1DeviceRoutingOspfv3AreaInterfaceidGetResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &V1DeviceRoutingOspfv3AreaInterfaceidGetResponse{}

// V1DeviceRoutingOspfv3AreaInterfaceidGetResponse struct for V1DeviceRoutingOspfv3AreaInterfaceidGetResponse
type V1DeviceRoutingOspfv3AreaInterfaceidGetResponse struct {
	Interfaces []string `json:"interfaces,omitempty"`
	PageInfo *CommonPageInfo `json:"pageInfo,omitempty"`
	// Reference to the resultset being queried, this should be sent by the service as part of a previous request and so can be opaque to the client.
	Token *string `json:"token,omitempty"`
}

// NewV1DeviceRoutingOspfv3AreaInterfaceidGetResponse instantiates a new V1DeviceRoutingOspfv3AreaInterfaceidGetResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewV1DeviceRoutingOspfv3AreaInterfaceidGetResponse() *V1DeviceRoutingOspfv3AreaInterfaceidGetResponse {
	this := V1DeviceRoutingOspfv3AreaInterfaceidGetResponse{}
	return &this
}

// NewV1DeviceRoutingOspfv3AreaInterfaceidGetResponseWithDefaults instantiates a new V1DeviceRoutingOspfv3AreaInterfaceidGetResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewV1DeviceRoutingOspfv3AreaInterfaceidGetResponseWithDefaults() *V1DeviceRoutingOspfv3AreaInterfaceidGetResponse {
	this := V1DeviceRoutingOspfv3AreaInterfaceidGetResponse{}
	return &this
}

// GetInterfaces returns the Interfaces field value if set, zero value otherwise.
func (o *V1DeviceRoutingOspfv3AreaInterfaceidGetResponse) GetInterfaces() []string {
	if o == nil || IsNil(o.Interfaces) {
		var ret []string
		return ret
	}
	return o.Interfaces
}

// GetInterfacesOk returns a tuple with the Interfaces field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1DeviceRoutingOspfv3AreaInterfaceidGetResponse) GetInterfacesOk() ([]string, bool) {
	if o == nil || IsNil(o.Interfaces) {
		return nil, false
	}
	return o.Interfaces, true
}

// HasInterfaces returns a boolean if a field has been set.
func (o *V1DeviceRoutingOspfv3AreaInterfaceidGetResponse) HasInterfaces() bool {
	if o != nil && !IsNil(o.Interfaces) {
		return true
	}

	return false
}

// SetInterfaces gets a reference to the given []string and assigns it to the Interfaces field.
func (o *V1DeviceRoutingOspfv3AreaInterfaceidGetResponse) SetInterfaces(v []string) {
	o.Interfaces = v
}

// GetPageInfo returns the PageInfo field value if set, zero value otherwise.
func (o *V1DeviceRoutingOspfv3AreaInterfaceidGetResponse) GetPageInfo() CommonPageInfo {
	if o == nil || IsNil(o.PageInfo) {
		var ret CommonPageInfo
		return ret
	}
	return *o.PageInfo
}

// GetPageInfoOk returns a tuple with the PageInfo field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1DeviceRoutingOspfv3AreaInterfaceidGetResponse) GetPageInfoOk() (*CommonPageInfo, bool) {
	if o == nil || IsNil(o.PageInfo) {
		return nil, false
	}
	return o.PageInfo, true
}

// HasPageInfo returns a boolean if a field has been set.
func (o *V1DeviceRoutingOspfv3AreaInterfaceidGetResponse) HasPageInfo() bool {
	if o != nil && !IsNil(o.PageInfo) {
		return true
	}

	return false
}

// SetPageInfo gets a reference to the given CommonPageInfo and assigns it to the PageInfo field.
func (o *V1DeviceRoutingOspfv3AreaInterfaceidGetResponse) SetPageInfo(v CommonPageInfo) {
	o.PageInfo = &v
}

// GetToken returns the Token field value if set, zero value otherwise.
func (o *V1DeviceRoutingOspfv3AreaInterfaceidGetResponse) GetToken() string {
	if o == nil || IsNil(o.Token) {
		var ret string
		return ret
	}
	return *o.Token
}

// GetTokenOk returns a tuple with the Token field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1DeviceRoutingOspfv3AreaInterfaceidGetResponse) GetTokenOk() (*string, bool) {
	if o == nil || IsNil(o.Token) {
		return nil, false
	}
	return o.Token, true
}

// HasToken returns a boolean if a field has been set.
func (o *V1DeviceRoutingOspfv3AreaInterfaceidGetResponse) HasToken() bool {
	if o != nil && !IsNil(o.Token) {
		return true
	}

	return false
}

// SetToken gets a reference to the given string and assigns it to the Token field.
func (o *V1DeviceRoutingOspfv3AreaInterfaceidGetResponse) SetToken(v string) {
	o.Token = &v
}

func (o V1DeviceRoutingOspfv3AreaInterfaceidGetResponse) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o V1DeviceRoutingOspfv3AreaInterfaceidGetResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Interfaces) {
		toSerialize["interfaces"] = o.Interfaces
	}
	if !IsNil(o.PageInfo) {
		toSerialize["pageInfo"] = o.PageInfo
	}
	if !IsNil(o.Token) {
		toSerialize["token"] = o.Token
	}
	return toSerialize, nil
}

type NullableV1DeviceRoutingOspfv3AreaInterfaceidGetResponse struct {
	value *V1DeviceRoutingOspfv3AreaInterfaceidGetResponse
	isSet bool
}

func (v NullableV1DeviceRoutingOspfv3AreaInterfaceidGetResponse) Get() *V1DeviceRoutingOspfv3AreaInterfaceidGetResponse {
	return v.value
}

func (v *NullableV1DeviceRoutingOspfv3AreaInterfaceidGetResponse) Set(val *V1DeviceRoutingOspfv3AreaInterfaceidGetResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableV1DeviceRoutingOspfv3AreaInterfaceidGetResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableV1DeviceRoutingOspfv3AreaInterfaceidGetResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableV1DeviceRoutingOspfv3AreaInterfaceidGetResponse(val *V1DeviceRoutingOspfv3AreaInterfaceidGetResponse) *NullableV1DeviceRoutingOspfv3AreaInterfaceidGetResponse {
	return &NullableV1DeviceRoutingOspfv3AreaInterfaceidGetResponse{value: val, isSet: true}
}

func (v NullableV1DeviceRoutingOspfv3AreaInterfaceidGetResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableV1DeviceRoutingOspfv3AreaInterfaceidGetResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


